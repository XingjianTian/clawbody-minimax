...
async def _analyze_image_with_openai(frame: np.ndarray, prompt: str) -> Optional[str]:
    """Analyze an image using MiniMax's Chat Completions API (vision-capable model).

    Uses the same MINIMAX_API_KEY already configured for the voice handler.
    ...
    try:
        ...
        if not api_key:
            logger.warning("No MINIMAX_API_KEY for vision analysis")
            return None
        ...
        logger.error("MiniMax vision analysis failed: %s", e)
        return None
...
async def _handle_camera(args: dict, deps: ToolDependencies) -> dict:
    ...
    2. MiniMax Vision API (cloud vision via MiniMax M2.7)
    ...
    try:
        ...
        # Option 2: Use MiniMax Vision API for image analysis
        logger.info("Using MiniMax Vision API for image analysis...")
        openai_description = await _analyze_image_with_openai(frame, vision_prompt)
        if openai_description:
            logger.info("MiniMax vision response: %s", openai_description[:100])
            return {
                "status": "success",
